...
install.packages(c("ggdag","dagitty"))
...
Coffee drinking has a relationship to cancer troguh the confounding. 
...
```{r}
dag_coffe_cancer <- coffee_cancer_dag %>% 
  tidy_dagitty() %>% 
  dag_paths()

# Shows the open path variable

ggdag_paths(coffee_cancer_dag) +
  theme_minimal() +
  labs(title = "Confounding pathways",
       subtilte = "No causal relation between coffe drinking and  cancer")
```
...
```{r}
ggdag_adjustment_set(dag_coffe_cancer,use_labels = "label",text = FALSE)

cancer ~ coffee + adictive
cancer ~ smoking
```


```{r}
coffee_cancer_dag_2 <- dagify(
  cancer ~ smoking,
  smoking ~ addictive,
  coffee ~ addictive,
  exposure = "coffee",
  ...
  latent = "addictive" # You dont have addictive behavior variable
)


ggdag_adjustment_set(coffee_cancer_dag_2)

```

```{r}
coffee_cancer_dag_3 <- dagify(
  cancer ~ smoking,
  smoking ~ addictive,
  coffee ~ addictive,
  exposure = "coffee",
  ...
  latent = c("addictive","smoking") # You dont have addictive behavior variable, we dont have sking too
)


ggdag_adjustment_set(coffee_cancer_dag_3)
