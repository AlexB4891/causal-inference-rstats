...
probability_weights <- propensity_model %>% 
  mutate(w_ate = (net/.fitted) + (( 1 - net)/( 1 - .fitted)))

outcome_model <- lm(data = probability_weights,
                    formula = health ~ net,
                    weights = w_ate) %>% 
  tidy()
...
boot_estimate <- int_t(.data = ipw_results,results) %>% 
  filter(term == "net")

```

# Stadard mean distance
...
```{r}
tip_c(effect = 3.318153	,smd = pull(smds,smd)) 
