...
2. For both data sets, use `standardized_model` (i call it `model_fit`) and `augment()` to get the predicted values. Use the `newdata` argument in `augment()` with the relevant cloned data set. Then, select only the fitted value. Rename `.fitted` to either `yes_extra_hours` or `no_extra_hours` (use the pattern `select(new_name = old_name)`).
3. Save the predicted data sets as`predicted_yes` and `predicted_no`.

```{r}
no_extra_hours <- seven_dwarfs %>% 
  mutate(extra_magic_morning == 0)

yes_extra_hours <- seven_dwarfs %>% 
  mutate(extra_magic_morning == 1)

predicted_yes <- model_fit %>% 
  augment(newdata = yes_extra_hours) %>%
  select(predicted_yes = .fitted)

predicted_no <- model_fit %>% 
  augment(newdata = no_extra_hours) %>%
  select(predicted_no = .fitted)
...
```{r}
bind_cols(predicted_yes,
          predicted_no) %>% 
  summarise(
    mean_yes = mean(predicted_yes,na.rm = T),
    mean_no = mean(predicted_no,na.rm = T),
